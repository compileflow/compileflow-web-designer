 ...
 */
var GlobalVarsDialog = function (graph, ui) {
    var divObj = $('<div></div>');

    //table
    var tableObj = $('<table cellspacing="0" cellpadding="0" style="border: 1px solid #ccc;"></table>');
    divObj.append(tableObj);

    //th
    var trObj = $('<tr></tr>');
    trObj.append(GlobalVarsDialog_createTd(''));
    trObj.append(GlobalVarsDialog_createTd('name'));
    trObj.append(GlobalVarsDialog_createTd('description'));
    trObj.append(GlobalVarsDialog_createTd('dataType'));
    trObj.append(GlobalVarsDialog_createTd('inOutType'));
    tableObj.append(trObj);

    divObj.append('<br/>');

    //add button
    var addBtn = mxUtils.button(mxResources.get('addRowText'), function (evt) {
        var trObj = $('<tr></tr>');
        trObj.append(GlobalVarsDialog_createATd());
        trObj.append(GlobalVarsDialog_createInputTd());
        trObj.append(GlobalVarsDialog_createInputTd());
        trObj.append(GlobalVarsDialog_createInputTd());
        trObj.append(GlobalVarsDialog_createInputTd());
        tableObj.append(trObj);
    });
    divObj.append(addBtn);

    //save button
    var saveBtn = mxUtils.button(mxResources.get('saveRoWText'), function (evt) {
        var data = [];
        ...
        graph.dataGlobalVars = CompileFlowUtils.globalVarsToStr(data);
        ui.hideDialog();
    });
    saveBtn.style.marginLeft = '20px';
    divObj.append(saveBtn);
    this.container = divObj[0];
};

var GlobalVarsDialog_createTd = function (value) {
    var tdObj = $('<td style="border: 1px solid #ccc; height:30px; width: 80px;">' + value + '</td>');
    return tdObj;
};

var GlobalVarsDialog_createInputTd = function () {
    var thObj = $('<td style="border:1px solid #ccc;height:30px;"><input style="width: 60px;"></input></td>');
    return thObj;
};
var GlobalVarsDialog_createATd = function () {
    var aObj = $('<a href="#" style="text-decoration: underline;cursor:hand;">' + mxResources.get('delRowText') + '</a>');
    aObj.click(function () {
        aObj.parent().parent().remove();
    });
    var tdObj = $('<td style="border:1px solid #ccc;height:30px;"></td>');
    tdObj.append(aObj);
    return tdObj;
};
